...
def clean_folder(path):
    # Додайте логіку для розбору папки тут
    # Наприклад, видалення зайвих файлів або сортування папки

    print(f"Cleaning folder: {path}")
    # Логіка для роботи з папкою
...
# Функція для створення каталогів за списком категорій
def create_directories(folder_path, categories):
    for category in categories:      # Створюємо каталоги для кожної категорії
        category_path = os.path.join(folder_path, category)
        os.makedirs(category_path, exist_ok=True)

# Функція для нормалізації імені файлу (транслітерація та заміна символів)
def normalize(name):      
    # Словник для транслітерації символів
...
# Функція для переміщення файлу до відповідного каталогу
...
# Функція для сортування файлів у папках
...
# Створення списків для відстеження відомих та невідомих розширень файлів
known_extensions = set()
unknown_extensions = set()

# Створення словника для зберігання файлів за категоріями
files_by_category = {
    'images': [],
    'video': [],
    'documents': [],
...
if __name__ == "__main__":
    # Перевірка правильності виклику скрипта
    if len(sys.argv) != 2:
        print("Usage: python script.py <folder_path>")
        sys.exit(1)
    # Отримання шляху до папки для сортування з аргументів командного рядка
    ...
    if not os.path.isdir(folder_to_sort):
        ...
        sys.exit(1)
    # Виклик функції для сортування папки з файлами
